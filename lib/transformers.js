    ...
    ADD_SOURCE_HASH: (resource)=>{
      ...
      if (resource.kind === CONSTANTS.KINDS.BUILD_CONFIG) {
        ...
        if (resource.spec.source.type === 'Git'){
          var branchName = 'HEAD'
          var revParseRef = `${branchName}:${contextDir}`
          var repositoryDir = client.cwd()
          //if it is referencing a repository that is not the current one, clone it to temporary location
          if (resource.spec.source.git.uri !== client.git.uri){
            branchName= resource.spec.source.git.ref || 'master'
            repositoryDir = `/tmp/${util.hashString(resource.spec.source.git.uri)}`
            revParseRef = `remotes/origin/${branchName}:${contextDir}`
            if (!fs.existsSync(repositoryDir)) {
              var girArgs=['clone',  '--no-checkout', '--single-branch', '--no-tags', '--depth', '1', `${resource.spec.source.git.uri}`, `${repositoryDir}`]
              logger.trace('>',  ['git'].concat(girArgs).join(' '))
              var gitCloneResult = spawnSync('git', girArgs, {cwd:'/tmp', encoding:'utf-8'})
              if (gitCloneResult.status != 0){
                throw new Error(`git clone failed for '${resource.spec.source.git.uri}'`)
              }
              logger.trace('>',  ['git'].concat(['fetch', '--depth', '1', '--no-tags', '--update-shallow', 'origin', `${branchName}:refs/remotes/origin/${branchName}`]).join(' '))
              spawnSync('git', ['fetch', '--depth', '1', '--no-tags', '--update-shallow', 'origin', `${branchName}:refs/remotes/origin/${branchName}`], {cwd:repositoryDir})
              logger.trace('>',  ['git'].concat(['checkout', `${branchName}:refs/remotes/origin/${branchName}`]).join(' '))
              spawnSync('git', ['checkout', `${branchName}:refs/remotes/origin/${branchName}`], {cwd:repositoryDir})
            }else{
              spawnSync('git', ['clean',  '-fd'], {cwd:repositoryDir})
              spawnSync('git', ['fetch', '--depth', '1', '--no-tags', '--update-shallow', 'origin', `${branchName}:refs/remotes/origin/${branchName}`], {cwd:repositoryDir})
            }
          }
          //git tree-hash is more stable than commit-hash
          var gitRevParseResult = spawnSync('git', ['rev-parse', revParseRef], {cwd:repositoryDir, encoding:'utf-8'})
          if (gitRevParseResult.status !=0 ){
            throw new Error(`git rev-parse failed for '${resource.spec.source.git.uri}' using 'git rev-parse ${revParseRef}' in '${repositoryDir}'`)
          }
          sourceHash = gitRevParseResult.stdout.toString().trim()
        }else if (resource.spec.source.type === 'Binary'){
          var rootWorkDir = spawnSync('git', ['rev-parse', '--show-toplevel'], {'cwd':client.cwd()}).stdout.toString().trim()
          ...
          var walk=(start, basedir)=>{
            ...
            if (stat.isDirectory()) {
              ...
              var hash = spawnSync('git', ['hash-object', '-t', 'blob', '--no-filters', start], {'cwd':client.cwd()}).stdout.toString().trim()

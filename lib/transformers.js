        ...
        if (resource.spec.source.type === 'Git'){
          ...
          if (resource.spec.source.git.uri !== client.git.uri){
            ...
            if (!fs.existsSync(repositoryDir)) {
              var gitCloneResult = spawnSync('git', ['clone',  '--no-checkout', '--single-branch', '--no-tags', '--depth', '1', `${resource.spec.source.git.uri}`, '-b', `${branchName}`, `${repositoryDir}`], {cwd:'/tmp', encoding:'utf-8'})

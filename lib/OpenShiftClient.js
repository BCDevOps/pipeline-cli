...
module.exports = class OpenShiftClient {
  constructor(options) {
    ...
    if (this._cwd == null) {
      this._cwd = spawnSync('git', ['rev-parse', '--show-toplevel'], {
        encoding: 'utf-8'
      }).stdout.trim();
    }
    const gitBranchName = spawnSync('git', ['name-rev', '--name-only', 'HEAD'], {
      encoding: 'utf-8'
    }).stdout.trim();
    const gitBranchRef = spawnSync('git', ['config', `branch.${gitBranchName}.merge`], {
      encoding: 'utf-8'
    }).stdout.trim();
    const gitRemoteName = 'origin';
    const gitRemoteUri = spawnSync('git', ['config', '--get', `remote.${gitRemoteName}.url`], {
      encoding: 'utf-8'
    }).stdout.trim();
  ...
  static splitNames(string, namespace) {
   ...
   * returns (array)
   */
  static unwrapOpenShiftList(object) {
    const result = [];
  ...
  static serializableMap(jsonString) {
  ...
  process(template, args) {
    ...
    if (util.isUrl(template)) {
      const proc = this._action(
        this.buildCommonArgs('process', ['-f', this.toFilePath(template)], args, { output: 'json' }) // eslint-disable-line comma-dangle
      );
  ...
  static toCommandArgsArray(args) {

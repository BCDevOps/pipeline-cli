  ...
  importImageStreams (objects, targetImageTag, sourceNamespace, sourceImageTag) {
    for (let i = 0; i < objects.length; i++) {
      ...
      if (util.normalizeKind(item.kind) === 'imagestream.image.openshift.io'){
        ...
        if (importAsTagging){
          this.raw ('tag', [`${sourceNamespace}/${item.metadata.name}:${sourceImageTag}`, `${item.metadata.name}:${targetImageTag}`], {'namespace':item.metadata.namespace})
          this.waitForImageStreamTag(`${item.metadata.name}:${targetImageTag}`)
        }else{
          const dockerImageReference1 = this.object(`${CONSTANTS.KINDS.IMAGE_STREAM_TAG}/${item.metadata.name}:${sourceImageTag}`, {'output':'jsonpath={.image.dockerImageReference}', 'namespace':sourceNamespace});
          const dockerImageName = dockerImageReference1.split('@')[1]
          ...
          const dockerImageRepository = this.object(`${CONSTANTS.KINDS.IMAGE_STREAM}/${item.metadata.name}`, {'output':'jsonpath={.status.dockerImageRepository}', 'namespace':item.metadata.namespace});

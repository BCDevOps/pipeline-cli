...
module.exports = class OpenShiftClientX extends OpenShiftClient {
  ...
  applyRecommendedLabels(resources, appName, envName, envId, instance) {
    ...
    const allLabels = {
      app: instance || `${commonLabels['app-name']}-${envLabels['env-name']}-${envLabels['env-id']}`
    };
    ...
    resources.forEach((item) => {
      ...
      if (item.kind === 'StatefulSet') {
        ...
        this.setLabel(item.template, { statefulset: item.metadata.name });
        ...
        this.setLabel(item, { deploymentConfig: item.metadata.name });
      }
    });
  ...
  copyRecommendedLabels(source, target) {
    ['app', 'app-name', 'env-name', 'env-id', 'github-repo', 'github-owner'].forEach(
      (labelName) => {
  ...
  importImageStreams(objects, targetImageTag, sourceNamespace, sourceImageTag) {
    for (let i = 0; i < objects.length; i += 1) {
      ...
      if (util.normalizeKind(item.kind) === 'imagestream.image.openshift.io') {
        const importAsTagging = this.options['dev-mode'] === 'true';
        if (importAsTagging) {
          this.raw(
            'tag',
            [
          ...
          const sourceDockerImageRepository = this.object(
            `${CONSTANTS.KINDS.IMAGE_STREAM}/${item.metadata.name}`,
            { output: 'jsonpath={.status.dockerImageRepository}', namespace: sourceNamespace }
          );
          const sourceImageName = this.object(
            `${CONSTANTS.KINDS.IMAGE_STREAM_TAG}/${item.metadata.name}:${sourceImageTag}`,
            { output: 'jsonpath={.image.metadata.name}', namespace: sourceNamespace }
          );
          ...
          this.delete(
            [
              `ImageStreamTag/${item.metadata.name}:temp1-${targetImageTag}`,
              `ImageStreamTag/${item.metadata.name}:temp2-${targetImageTag}`
            ],
            { 'ignore-not-found': 'true', wait: 'true', namespace: targetNamespace }
          );
          const currentTag = this.get(
            [`${CONSTANTS.KINDS.IMAGE_STREAM_TAG}/${item.metadata.name}:${targetImageTag}`],
            { 'ignore-not-found': 'true' }
          );
          let currentImageName = '';
          if (currentTag.length > 0) {
            currentImageName = currentTag[0].image.metadata.name;
          }

          if (currentImageName !== sourceImageName) {
            // eslint-disable-next-line prettier/prettier,max-len
            logger.info(`Tagging '${item.metadata.name}:${targetImageTag}' from '${CONSTANTS.KINDS.IMAGE_STREAM_TAG}/${item.metadata.name}:${sourceImageTag}' @  ${sourceImageName}`);
            this.raw('import-image', [`${item.metadata.name}:temp1-${targetImageTag}`], {
              from: dockerImageReference1,
              confirm: 'true',
              insecure: 'true',
            ...
            const targetDockerImageRepository = this.object(
              `${CONSTANTS.KINDS.IMAGE_STREAM}/${item.metadata.name}`,
              { output: 'jsonpath={.status.dockerImageRepository}', namespace: targetNamespace }
            );
            this.raw('import-image', [`${item.metadata.name}:temp2-${targetImageTag}`], {
              from: `${targetDockerImageRepository}@${sourceImageName}`,
              confirm: 'true',
              insecure: 'true',
              'reference-policy': 'local',
              namespace: targetNamespace
            ...
            this.raw(
              'tag',
              [
                `${item.metadata.name}@${sourceImageName}`,
                `${item.metadata.name}:${targetImageTag}`
              ],
              { namespace: targetNamespace, 'reference-policy': 'local' }
            );
            this.waitForImageStreamTag(`${item.metadata.name}:${targetImageTag}`);
            this.delete(
              [
            ...
            // eslint-disable-next-line prettier/prettier,max-len
            logger.info(`ImageStreamTag '${item.metadata.name}:${targetImageTag}' is already up-to-date '${item.metadata.name}/${currentImageName}'`);
          }
        }
      }
    }

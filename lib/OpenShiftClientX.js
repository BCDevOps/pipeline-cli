  ...
  */
    ...
    for (let i = 0; i < objects.length; i++) {
      ...
      if (util.normalizeKind(item.kind) === 'imagestream.image.openshift.io'){
        ...
        if (importAsTagging){
          ...
          const currentTag = this.get([`${CONSTANTS.KINDS.IMAGE_STREAM_TAG}/${item.metadata.name}:${targetImageTag}`], {'ignore-not-found':'true'})
  ...
  async applyAndDeploy (resources, appName) {
    //return async function (resources) {
      ...
      //TODO: When there is an HorizontalPodAutoscaler, we need to update the dc.replicas to match hpa.status.desiredReplicas (existing) or hpa.spec.minReplicas (new)

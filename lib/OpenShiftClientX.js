...
class OpenShiftClientX extends OpenShiftClient {
  ...
  */
  ...
  startBuildIfNeeded(buildConfig) {
    ...
    if (buildConfig.spec.source.type === 'Binary' || (this.options['dev-mode'] === 'true' && !(buildConfig.spec.source.type === 'Dockerfile' && buildConfig.spec.strategy.type === 'Docker')) ) {
      util.execSync('gtar',['--version'],{
        //stderr: [process.stderr],
        //shell: true,
        //has_errors: true,
        encoding: 'utf-8'
      })
      //console.log(gtarOutput)
      if (fs.existsSync(tmpfile)) {
        fs.unlinkSync(tmpfile);
      }
      const procArgs = ['-chf', tmpfile, buildConfig.spec.source.contextDir];
      const procOptions = { cwd: this.cwd(), encoding: 'utf-8' };
      util.execSync('gtar', procArgs, procOptions);
      Object.assign(args, { 'from-archive': tmpfile });
      hashData.source=util.execSync('shasum',[tmpfile],procOptions).stdout.split(' ')[0]
      // Place holder for getting hash of an archive
      // util.hashArchive(tmpfile).then((hash)=>{

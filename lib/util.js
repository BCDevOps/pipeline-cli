...
function parseArgumentsFromArray(...argv) {
  ...
  //git.branch.merge=git.branch.name
  ...
  if (git.http_url == null) {
   ...
   if (git.http_url.startsWith('https://github.com') && git.branch.merge == null) {
    // eslint-disable-next-line prettier/prettier
  ...
  if (git.http_url.startsWith('https://bwa.nrs.gov.bc.ca/int/stash') && !git.branch.merge) {
    git.branch.merge = git.branch.name;
  }

...
function parseArgumentsFromArray(...argv) {
  ...
  if (git.branch.name == null) {
    // eslint-disable-next-line prettier/prettier
    git.branch.name = execSync('git', ['name-rev', '--name-only', 'HEAD'], { encoding: 'utf-8' }).stdout.trim();
  }
  ...
  if (git.branch.remote == null) {
    // eslint-disable-next-line prettier/prettier
    git.branch.remote = execSync('git', ['config', `branch.${git.branch.name}.remote`], { encoding: 'utf-8' }).stdout.trim();
  }
  if (git.url == null) {
    // eslint-disable-next-line prettier/prettier
